        ...
        const data = {
          ...
          axios.post(`api/contact-us`,data).then(res =>{
            if(res.data.success === true){
                ...
                Swal("Success", "Votre message a été bien envoyé "+ res.data.data.nom+' '+res.data.data.prenom ,"success")
                setTimeout(function(){
                ...
                Swal("Erreur", "Champ(s) vide(s) " ,"error")
            }
          }) 
                ...
                <Grid.Column floated='right' width={8}>
                    <form onSubmit={contactSubmit}>
                        <div style={{display:'grid',rowGap:'1rem', marginTop:"50px"}}>
                            ...
                            <FormControl fullWidth variant="outlined" color="success">
                                <InputLabel htmlFor="Email" sx={{width:"200px"}} >Adresse E-mail</InputLabel>
                                <OutlinedInput id="email" type='email' name="email" value={contactInput.email}
                                    onChange={handleInput} placeholder='Entrer votre adresse e-mail'
                                    startAdornment={<InputAdornment position="start"><AlternateEmailIcon/></InputAdornment> }  
                                    error={!!contactInput.error_list.email} label="Adresse E-mail" 
                                />
                                <FormHelperText error={true}>
                            ...
                            <FormControl fullWidth variant="outlined" color="success">
                                <InputLabel htmlFor="numero_telephone" sx={{width:"200px"}} >Numéro téléphone</InputLabel>
                                <OutlinedInput id="numero_telephone" type='text' name="numero_telephone" value={contactInput.numero_telephone}
                                    onChange={handleInput} placeholder='Entrer votre numéro téléphone'
                        ...
                        <br/>
                        <Button type='submit' color='success' variant="contained" sx={{float:" right"}}>
                            Envoyer 
                        </Button>
                    </form>
                </Grid.Column>
