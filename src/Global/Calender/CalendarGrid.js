...
const GridWrapper = styled.div`
  ...
  grid-gap: 1px;
  background-color: ${props => props.isHeader ? '#FFFFF' : 'grey'};
	${props => props.isHeader && `border-bottom: 2px solid #4D4C4D`}
`;

const CellWrapper = styled.div`
	min-height: ${props => props.isHeader ? 40 : 70}px;
	min-width: 50px;
	background-color: ${props => props.isWeekday ? 'white' : 'white'};
	color: ${props => props.isSelectedMonth ? 'black' : 'darkgrey'};
`;
