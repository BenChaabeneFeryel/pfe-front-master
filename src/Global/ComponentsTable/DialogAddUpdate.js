  ...
  return (
    <div>
      <Dialog open={open} onClose={handleClose} aria-labelledby="alert-dialog-title"  aria-describedby="alert-dialog-description"  fullWidth>
        <DialogTitle id="alert-dialog-title" sx={{backgroundColor: 'white', color:"green" , textAlign:"center", fontWeight:'bold'}}>
          {id ? "Modification" : "Création d'une nouvelle instance"}
        </DialogTitle>
          ...
          <form >  
            ...
            {show.length!==0?(show.map((sh, key) =>  
              ((sh[1]!=="id" 
                ...
                && sh[1]!=="quantite_total_collecte_canette" 
                && sh[1]!=="created_at" 
                && sh[1]!=="updated_at" 
                && sh[1]!=="photo")?(
                <>
                  <TextField key={key} id={sh[1]} value={data[sh[1]]} onChange={e=>onChange(e)} placeholder={sh[1]}  
                    error={!!validation[sh[1]]} label={sh[1]} variant="outlined" margin="dense" fullWidth />
                  <FormHelperText error={true}>
                    {validation[sh[1]]}        
                  </FormHelperText>                  
                </>): null)                 
