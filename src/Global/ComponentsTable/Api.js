...
export default function Api({initialValue, url, columnDefs, show}) {
  ...
  const handleDelete = (oldData) => {
    ...
    Swal("Suppression", "Ligne supprimé avec succés!" ,"success")
    if (confirm) {
      if(localStorage.getItem('auth_token_responsable')){
  ...
  const handleFormSubmit= (e) =>  {
    ...
    if(columnDefs[0].field==='id'){
        tableColumn=  columnDefs.concat(
          { headerName: "Date d'ajout", field: "created_at", type: ['dateColumn', 'nonEditableColumn'], maxWidth: 200, minWidth:180 },
          { headerName: "Date de modification", field: "updated_at", type: ['dateColumn', 'nonEditableColumn'], maxWidth: 200, minWidth:180  },
          { headerName: "Actions",sortable:false,filter:false,maxWidth: 180,pinned: 'right', cellRenderer: (params) => 
            <div>
